...
# Les 2 lignes suivantes sont de la config, ça ne doit pas être réutilisé à chaque inférence (elle n'est utilisée qu'une fois, pour configurer le réseau de neurones)
warmup_input = tf.random.uniform(input_shape, 0, 1.0) # Génère une image aléatoire de la forme de l'image en entrée
warmup_logits = classifier(warmup_input).numpy() # Envoie l'image générée dans le réseau pour avoir des premiers poids et configurer le réseau
